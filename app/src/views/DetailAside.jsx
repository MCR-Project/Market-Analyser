...
export const DetailAside = memo(function DetailAside({ etf, tickers, selected, onSelect, correlationData, sectorData }) {
  ...
  return (
    <aside className="w-[320px] flex-none bg-[var(--bg-1)] border border-[var(--border)] rounded-[var(--radius-lg)] shadow-[var(--shadow-sm)] p-[18px] self-start sticky top-4 max-h-full overflow-hidden flex flex-col">
      {selData ? (
        <>
          ...
          <div className="flex gap-2.5 mt-4 mb-1 flex-none">
          ...
          <div className="font-[var(--font-mono)] text-[11px] text-[var(--fg-2)] mt-4 mb-2.5 flex-none">CORRELATION TO PEERS</div>
          <div className="corr-scroll min-h-0 overflow-y-auto flex flex-col gap-2 pr-1 -mr-1">
            {peers.map(p => (
              <button key={p.ticker} onClick={() => onSelect(p.ticker)} className="flex items-center gap-2.5 bg-transparent border-none p-0 cursor-pointer text-left flex-none">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto corr-scroll">
